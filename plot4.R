...
# 2. Open PNG device; create "plot4.png" in working directory
png(filename = "plot4.png", width = 480, height = 480, units = "px")

# 3. Create a Date_Time
Date_Time <- as.POSIXlt(paste(as.Date(data$Date, format="%d/%m/%Y"), data$Time, sep=" "))

# 4. Create a frame
par(mfcol=c(2, 2), cex=0.6, mar=c(4, 4, 1, 1))
...
## Third Graph
plot(Date_Time, data$Voltage,type="l", xlab='datetime',ylab="Valtage")

## Fourth Graph
plot(Date_Time, data$Global_reactive_power,type="l", xlab='datetime',ylab="Global_active_power")
